...
def train():
    ...
    save_dir = os.path.join(cfg.checkpoint_dir, f'training_log_center_only_{timestamp}')
    ...
    for epoch in range(cfg.epochs):
        ...
        for imgs, labels in train_loader:
            ...
            # 只计算中心损失
            loss = center_loss_fn(hash_codes, labels)
        ...
        if val_acc > best_acc:
            ...
            torch.save({
                ...
                'val_acc': val_acc,
            }, os.path.join(cfg.checkpoint_dir, 'best_model_center_only.pth'))
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss Curve (Center Loss Only)')
    plt.legend()
    plt.grid(True)
