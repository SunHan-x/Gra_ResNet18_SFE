...
def train():
    ...
    save_dir = os.path.join(cfg.checkpoint_dir, f'training_log_quant_only_{timestamp}')
    ...
    for epoch in range(cfg.epochs):
        ...
        for imgs, labels in train_loader:
            ...
            # 只计算量化损失
            loss = quantization_loss(hash_codes)
        ...
        if val_acc > best_acc:
            ...
            torch.save({
                ...
                'val_acc': val_acc,
            }, os.path.join(cfg.checkpoint_dir, 'best_model_quant_only.pth'))
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss Curve (Quantization Only)')
    plt.legend()
    plt.grid(True)
